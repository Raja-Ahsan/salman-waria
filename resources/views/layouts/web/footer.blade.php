...
<script>
// ─── SECTION SCROLL (home page deep links: /impact, /ai-ventures, etc.) ───
function swScrollToSectionTarget() {
  if (typeof window.swScrollToSectionNow === 'function') {
    return window.swScrollToSectionNow();
  }
...
function swScrollToSectionTargetWithRetries() {
  if (!window.__SW_SCROLL_TO_ID) return;
  [0, 50, 150, 300, 600, 1000, 1500, 2200, 3000].forEach(function (ms) {
    setTimeout(function () {
      if (swScrollToSectionTarget() && typeof window.swRevealAfterSectionScroll === 'function') {
        window.swRevealAfterSectionScroll();
      }
      if (ms >= 1500 && typeof ScrollTrigger !== 'undefined') ScrollTrigger.refresh();
    }, ms);
  });
}
...
function swStartHomePage() {
  ...
  swScrollToSectionTargetWithRetries();
}
...
(function() {
  ...
  if (window.__SW_SCROLL_TO_ID) {
    ...
    if (document.readyState === 'loading') {
      document.addEventListener('DOMContentLoaded', swStartHomePage);
    } else {
      swStartHomePage();
    }
    return;
...
if (contactForm) {
  contactForm.addEventListener('submit', async function(e) {
    ...
    try {
      const submitUrl = @json(route('contact.submit'));
      ...
      if (data.csrf_token) {
        if (tokenField) tokenField.value = data.csrf_token;
        const laravelTok = contactForm.querySelector('input[name="_token"]');
        if (laravelTok) laravelTok.value = data.csrf_token;
        const metaUp = document.querySelector('meta[name="csrf-token"]');
        if (metaUp) metaUp.setAttribute('content', data.csrf_token);
      }
      ...
      if (data.ok) {
        ...
        if (data.csrf_token) {
          if (tokenField) tokenField.value = data.csrf_token;
          const laravelToken = contactForm.querySelector('input[name="_token"]');
          if (laravelToken) laravelToken.value = data.csrf_token;
...
async function handleNewsletterSubmit(e) {
  ...
  try {
    const submitUrl = @json(route('newsletter.submit'));
    const csrfMeta = document.querySelector('meta[name="csrf-token"]');
    const csrfToken = csrfMeta ? csrfMeta.getAttribute('content') : '';
    const res = await fetch(submitUrl, {
      method: 'POST',
